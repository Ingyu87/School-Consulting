...
1. PDF 공식 양식 구조를 앱 데이터 모델로 변환
2. 고일초 CSV 데이터를 초기값으로 탑재
3. 진단 결과 시각화와 분석 문구 편집
4. 심층면담지 입력 폼 구현
5. 운영계획서 입력 폼 구현
6. 연수 구성 검증 엔진 구현
7. DOCX 다운로드 구현
...
초기 구현에서는 고일초 CSV 분석 결과를 코드 내 JSON으로 탑재한다. 이후 파일 업로드를 지원한다.
...
고일초 초기 분석:
- 학생 AI·디지털 기초 소양이 낮은 편
- 학부모 이해가 낮은 편
- 기기 성능/활용 환경 확인 필요
- 교과별 수업실천은 실습형 연수와 연결 가능
...
- `서울고일초_심층면담지_YYYYMMDD.docx`
- `서울고일초_운영계획서_YYYYMMDD.docx`
